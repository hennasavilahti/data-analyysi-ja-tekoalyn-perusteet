...
emp_min_sal = df['salary'].min()
emp_max_sal = df['salary'].max()
emp_mean_sal = df['salary'].mean()

tuotekehitys_mean_sal = df.loc[df['dname'] == 'Tuotekehitys', 'salary'].mean()

no_second_phone = df['phone2'].isna().sum()

df['age'] = (datetime.now() - pd.to_datetime(df['bdate'])) // timedelta(days=365.2425)

bins2 = list(range(15,75,5))
    
labels = bins2[1:]
df['age_group'] = pd.cut(df['age'], bins=bins2, labels=labels, right=False)
df['age_group'] = df['age_group'].astype(int)

new_df = df[['salary', 'age', 'gender']].copy()
